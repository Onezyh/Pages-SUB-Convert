...
### 1. 部署 Cloudflare Pages：
   ...
   - 在 Cloudflare Pages 控制台中选择 `连接到 Git`后，选中 `Pages-SUB-Convert`项目后点击 `开始设置`。

### 2. 给 Pages绑定 自定义域：
   - 在 Pages控制台的 `自定义域`选项卡，下方点击 `设置自定义域`。
   - 填入你的自定义次级域名，注意不要使用你的根域名，例如：
     您分配到的域名是 `fuck.cloudns.biz`，则添加自定义域填入 `sub.fuck.cloudns.biz`即可；
   - 按照 Cloudflare 的要求将返回你的域名DNS服务商，添加 该自定义域 `sub`的 CNAME记录 `Pages-SUB-Convert.pages.dev` 后，点击 `激活域`即可。
...
### 1. 部署 Cloudflare Worker：
   ...
   - 将 [worker.js](https://github.com/Onezyh/Pages-SUB-Convert/blob/mian/_worker.js)  的内容粘贴到 Worker 编辑器中。
...
# 感谢
[mianayang](https://github.com/mianayang/myself/blob/main/cf-workers/sub/sub.js)、[ACL4SSR](https://github.com/ACL4SSR/ACL4SSR/tree/master/Clash/config)、[肥羊](https://github.com/youshandefeiyang/sub-web-modify)、[cmliu](https://github.com/cmliu)
